...
def create_svm_graph(training_points):
    ...
    training_point_x_vals = list(map(get_x, training_points))
    training_point_y_vals = list(map(get_y, training_points))
    ...
    def update_svm_plot(svm, final_update=False):
        ...
        sv_circles.set_xdata(list(map(get_x, support_vectors)))
        sv_circles.set_ydata(list(map(get_y, support_vectors)))
