...
from boards import *
from lab2 import (next_boards_connectfour, is_game_over_connectfour,
...
def heuristic_connectfour_4_testanswer(val, original_val = None) :
    return (all([v < 0 for v in val[:2]])
            and all([v > 0 for v in val[2:]])
            and all([abs(v) < 1000 for v in val])
            and all([val[i] < val[i+1] for i in range(3)]))
make_test(type = 'MULTIFUNCTION',
...
def progressive_0_testanswer(val, original_val = None) :
    ...
    return (all(map(is_dfs_return_type, h))
            and [11,10,10] == [x[1] for x in h] and [4,5,6] == [x[2] for x in h])
...
def progressive_1_testanswer(val, original_val = None) :
    ...
    h = val.history
    return (all(map(is_dfs_return_type, h))
            and [4, -2, 5, -3, 20] == [x[1] for x in h] and [7,34,176, 695, 2259] == [x[2] for x in h])
