...
class BayesNet:
    ...
    def get_parents(self, var) :
        ...
        if var not in self.variables:
            raise LookupError(str(var)+" is not a variable in this network.")
        return set([w for w in self.variables if var in self.adjacency.get(w,[])])
    ...
    def unlink(self, var1, var2=None) :
        ...
        if var2 == None:
            list(map(lambda v2: self.unlink(var1, v2), self.variables))
            return self
    ...
    def CPT_print(self, var=None) :
        ...
        rows = [row for row in self.conditional_probability_table if list(row[0][0].keys())[0]==var]
    ...
    def combinations(self, variables, constant_bindings=None) :
        ...
        unbound_variables = [v for v in variables if v not in constant_bindings]
    ...
    def topological_sort(self, variables=None) :
        ...
        visited = []
        while agenda :
            agenda = [x for x in agenda if x not in visited]
            if agenda :
